...
2. 替换自动化脚本
	1. Linux
		...
		main
		```
	1. Win（可能存在一些编码的问题，如果使用GB 2312或者 UTF-8均无法处理请使用后面的英文版本）
		```powershell
		@echo off
		...
		:local_option
		    ...
		    exit /b 0
		```
	2. 安卓（1.16.0官方版本支持导入自建planet但是需要将planet转为base64编码，但是不支持moon入轨)
		可以使用zerotier-fix软件即可实现，或者参考官方教程（[导入自建planet](https://docs.zerotier.com/roots/)）
	3. 文件下载
		1. linux ![](planet_update.sh)
		2. win ![win_ch](planet_update.bat)![win_en](planet_update_en.bat)
		3. Android
